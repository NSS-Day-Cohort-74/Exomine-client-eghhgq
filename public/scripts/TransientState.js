...
export const purchaseMineral = async () => {
   ...
   for (const state of purchases) {
	await updateFacilityMineral(state);
	await updateColonyMinerals(state);
   }
*/
...
const updateFacilityMineral = async () => {
	const fetchFacilityMinerals = await fetch(
	...
	const filteredFacilityMinerals = facilityMinerals.filter(
		(fm) => fm.facilityId === state.selectedFacility,
	);
	const obj = filteredFacilityMinerals.find(
		(fm) => fm.mineralId === state.selectedMineral,
	);
	const endpointId = obj.id;
	obj.amount--;

	const res = await fetch(
		"http://localhost:3000/facilityMinerals/" + endpointId,
		{
			method: "PUT",
			headers: {
...
const updateColonyMinerals = async () => {
	const fetchColonyMinerals = await fetch(
	...
	const filteredColonyMinerals = colonyMinerals.filter(
		(cm) => cm.colonyId === state.selectedColony,
	);
	let method = "POST";
	let object = {};
	let endpoint = "";
	if (!filteredColonyMinerals) {
		...
		object = filteredColonyMinerals.find(
			(fm) => fm.mineralId === state.selectedMineral,
		);
		if (!object) {
			method = "POST";
