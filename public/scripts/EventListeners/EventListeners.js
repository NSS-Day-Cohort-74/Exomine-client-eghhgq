...
const govHandler = async ({
    target: {
        ...
        if (name === "governers") {
         ...
         document.querySelector("#colony-minerals").innerHTML = await genColonyHTML(options.selectedIndex)
        }
};
const facilityHandler = async (event) => {
    if (event.target.name === "facilities") {
     console.log(event.target.options[event.target.options.selectedIndex].dataset.name)
     document.querySelector("#facility-minerals").innerHTML = await genFacilityHTML(event.target.options.selectedIndex)
    }
};